...
function buildObservation(results: RankedLabel[]): VisualObservation {
  ...
  const noPersonEvidence = adult.value === "none" && adult.confidence >= 0.92;
  const explicitEvidence = safety.value === "explicit" && safety.confidence >= 0.95;
  // The user's explicit 18+ attestation is the primary age confirmation.
  // CLIP is not an age estimator, so its apparent-age label never blocks a result.
  const adultConfidence = 0.95;
  ...
  const observation: VisualObservation = {
    ...
    evidence: {
      ...
      missingTraits,
      adultConfidence,
      appropriate: !noPersonEvidence && !explicitEvidence,
    },
  };
